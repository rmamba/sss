...
class TestRemoveNoConfig(unittest.TestCase):
  test_data = None
  ...
  def test_01_no_arguments(self):
    SSS3(['sss3.py', 'remove'])
    self.assertEqual(sys.stdout.getvalue(), 'No file specified! Specify which file to remove from the repository or use help command to find out more.\n')
    data = None
  ...
  def test_02_no_config_error(self):
    SSS3(['sss3.py', 'remove', 'test'])
    self.assertEqual(sys.stdout.getvalue(), 'Configuration setup missing, please run init command first.\n')
    data = None
    with open(self.CONTENT_FILE, 'r') as f:
      data = f.read()
    self.assertEqual(data, '{"test": {"size": 10}}')

class TestRemoveWithConfig(unittest.TestCase):
    test_data = None
    TEST_FILE = 'testdata.json'
    FOLDER_NAME = '.sss3'
    CONFIG_FILE = './{0}/config.json'.format(FOLDER_NAME)
    ...
    def test_04_add_two_files(self):
        ...
        with open(self.CONTENT_FILE, 'r') as f:
            data = f.read()
        json_data = json.loads(data)
        self.assertEqual(json_data.keys(), [u'test', u'test2'])
        self.assertEqual(json_data[u'test'][u'size'], 10)
